...
#	This seems to work on all pip versions
	...
	if ! $(PYTHON) -m pip install . ; then $(PYTHON) -m pip install . ; fi
...
# 	Perhaps we should link these as well
	...
	$(PYTHON) -m pip uninstall -y mininet || true
	if ! $(PYTHON) -m pip install -e . --no-binary :all: ; then $(PYTHON) -m pip install -e . --no-binary --break-system-packages :all: ; fi
